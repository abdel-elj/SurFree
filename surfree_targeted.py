...
class SurFree():
    ...
    def __call__(self, model, X, labels, starting_points=None, **kwargs):
        ...
        # Get target labels for targeted attack (target class = 577)
        self.target_labels = torch.tensor([577] * X.shape[0], device=X.device, dtype=torch.long)
        assert self.target_labels is not None, "Target labels required for targeted attack."

        # Get Starting Point / start with orthogonal noise toward target
        self.best_advs = get_init_with_noise(model, X, labels) if starting_points is None else starting_points
        self.X = X

        # Check if X already classified as target class
        self._images_finished = model(X).argmax(1) == self.target_labels
        print("Already targeted advs: ", self._images_finished.cpu().tolist())

        self.best_advs = torch.where(atleast_kdim(self._images_finished, len(X.shape)), X, self.best_advs)

        # Step 1: Initialize directions (search for direction towards target class)
        ...
        found_target = False
        past_best_advs = self.best_advs
        class_best_advs = self.target_labels
        same_as_past = 0
        for step_i in range(self._steps):
            ...
            # Targeted success: candidate predicts target label and is closer than current best
            preds = model(best_candidates).argmax(1)
            is_target_hit = preds == self.target_labels
            closer = distance(best_candidates, self.X) < distance(self.best_advs, self.X)
            is_success = is_target_hit & closer

            # If we find a better candidate (closer to target class), update the adversarial sample
            ...
            # Check if the target class has been reached
            if is_target_hit.all() and not found_target:
                print("Target class found!")
                found_target = True

            # Step 2: If we found the target class, apply binary search for refinement
            if found_target:
                self.best_advs = self._binary_search(self.best_advs, boost=True)

            current_preds = model(self.best_advs).argmax(1)
            self._images_finished = current_preds == self.target_labels
            #class_best_advs = model(self.best_advs).argmax(1)
            print("Best Advs distance:", distance(X, self.best_advs).cpu().numpy())
            print("Best Advs class:", current_preds)
            if (distance(X, self.best_advs).cpu().numpy() == distance(X, past_best_advs).cpu().numpy()) and self._images_finished and same_as_past>3:
                break;
            # Stop if all images are classified as target class
        ...
        if self.target_labels != class_best_advs:
            self.best_advs = past_best_advs
        # Final refinement step if binary search is enabled
    ...
    def _get_candidates(self):
        ...
        while (epsilon == 0).any():
            ...
            if( indexes == []):
                print("Refreshing directions...")
                indexes = random.sample(range(len(self.X)), k=min(5, len(self.X)))
            new_directions = self._basis.get_vector(self._directions_ortho, indexes = indexes)
    ...
    def _binary_search(self, perturbed: torch.Tensor, boost= False) -> torch.Tensor: # Refine Adversarial example using binary
        ...
        if boost:
            ...
            boost_perturbed = torch.where(is_advs, boost_vec, perturbed)
        else:
            originals = self.X

        # use this variable to check when mids stays constant and the BS has converged
        if self._model(boost_perturbed).argmax(1) == self.target_labels:
            perturbed = boost_perturbed
        iteration = 0
        is_targeted = True
        last_targeted = perturbed
        while torch.any(highs - lows > thresholds) and iteration < self._BS_max_iteration:
            ...
            preds = self._model(mids_perturbed).argmax(1)
            is_targeted = preds == self.target_labels
            if is_targeted:
                last_targeted = mids_perturbed
            is_adversarial_ = self._is_adversarial(mids_perturbed) & is_targeted
            highs = torch.where(is_adversarial_, mids, highs)
            lows = torch.where(is_adversarial_, lows, mids)
        if not (is_targeted):
            return last_targeted
